...
public class GreatestNumberOfCandies {
    ...
    public static List<Boolean> kidsWithCandies(int[] candies, int extraCandies) {
        List<Boolean> largestNumberOfCandies = new ArrayList<>();
        int mostCandies = candies[0];
        for(int i = 0; i < candies.length - 1; i++){
            mostCandies = (mostCandies <= candies[i+1]) ? candies[i+1] : mostCandies;
            System.out.println(candies[i+1]);
        }

        for(int i = 0; i < candies.length; i++){
            if((candies[i] + extraCandies) < mostCandies){
                largestNumberOfCandies.add(false);
            } else{
                largestNumberOfCandies.add(true);
            }
        }
        return largestNumberOfCandies;
    }
}
